...
export default {
    ...
    handler: throttle(async function (type, disNum, cart) {
      ...
      switch (type) {
        ...
        case "minus":
          ...
          break;
        // 用户输入进来的最终值，如果是非法的（带有汉字|出现负数），带给服务器数字零
        case "change":
          if (isNaN(disNum) || disNum < 1) {
            disNum = 0;
          } else {
            // 正常的数字，但是可能是小数，带给服务器是变化的量  用户输入进来的 - 产品起始个数
  ...
  computed: {
    ...
    // 判断是否全选
    isAllCheck() {
      // 遍历数组里面原理，只要全部元素isChecked属性都为1，就返回true，只要有一个返回的不是1 ，就返回false
